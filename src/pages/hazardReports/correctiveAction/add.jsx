...
import { addCorrectiveAction, detailsCorrectiveAction, updateCorrectiveAction } from 'provider/features/hazardReport/hazardReport.slice';
...
const Add = () => {
  ...
  const { data: reportData } = useSelector((state) => state.hazardReport.reportData);
  const { isLoading: addLoader } = useSelector(
    (state) => state.hazardReport.addCorrectiveAction);
  const { isLoading: updateLoader } = useSelector(
    (state) => state.hazardReport.updateCorrectiveAction);
  const { data: detailsData } = useSelector(
    (state) => state.hazardReport.detailsCorrectiveAction);
  let data = id ? detailsData : reportData;
  ...
  const handleSubmit = async (values) => {
    ...
    if (id) {
      ...
      dispatch(updateCorrectiveAction({
        data: values,
        successCallback: () =>
          navigate(`/hazard-reports/corrective-actions/${hazardId}`)
      }));
      return
    }
    ...
    dispatch(addCorrectiveAction({
      data: values,
      successCallback: () =>
        navigate(`/hazard-reports/corrective-actions/${hazardId || data.hazardId}`)
    }));
  };
  ...
  useEffect(() => {
    if (id) {
      const data = { hazardId, correctiveActionId: id }
      dispatch(detailsCorrectiveAction({ data, successCallback }));
    }
  }, [dispatch, id, hazardId]);
    ...
    <StyledCorrectiveAction>
      ...
      <div className="corrective_form">
        <div className="corrective_form_action-category">
          <p>
            Action Category: Hazard Report &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            Raised By User: {data?.informantName ?? data?.raisedByUser}
          </p>
          <p>
            Report ID: {data?.hazardId}
